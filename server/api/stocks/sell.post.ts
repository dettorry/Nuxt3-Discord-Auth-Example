...
type SellBody = {
  ...
  symbol?: string;
  quantity?: number;
  unitPrice?: number; // rounded; required for now
  reason?: string;
};
...
export default defineEventHandler(async (event: H3Event) => {
  ...
  const body = (await readBody(event)) as SellBody;
  const {
    userId,
    guildId: bodyGuildId,
    symbol: bodySymbol,
  ...
  const unitPrice = Math.round(priceInput);
  const totalProceeds = unitPrice * quantity;

  // Credit directly to bank
  const client = new Client(config.UNBELIEVABOAT_TOKEN);
  try {
    const reason = bodyReason || `Vente de ${quantity} actions ${symbol}`;
    await client.editUserBalance(guildId, userId, { bank: totalProceeds }, reason);
    return {
      ok: true,
      symbol,
      quantity,
    ...
    throw createError({ statusCode: 502, statusMessage: err?.message || 'Vente impossible' });
  }
});
